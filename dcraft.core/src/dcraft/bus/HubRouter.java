...
import dcraft.schema.SchemaManager.OpInfo;
...
public class HubRouter {
				...
				.withWork(task -> {			
					FuncCallback<UserContext> fcb = new FuncCallback<UserContext>() {
						...
						public void callback() {
				            ...
				            if (Logger.isDebug())
				            	task.debug("Message being processed for : " + srv);
				            ...
				            if (Logger.isDebug())
				            	task.debug("Message being validated for : " + srv);
				            ...
				            if (Logger.isDebug())
				            	task.debug("Message being handled for : " + srv);
    	...
    	if (this.gateway && !msg.isVerifyRequest() && !looksLikeReply) {
			...
			if (!isguest) {
				...
				if (!op.isTagged("Gateway")) {
			        ...
			        dcraft.session.Session us = Hub.instance.getSessions().lookup(sid);
	...
	public OperationResult deliverMessage(final StreamMessage msg) {
        ...
        if (this.local) {    	
    		...
    		final dcraft.session.Session sess = Hub.instance.getSessions().lookup(sessid);
