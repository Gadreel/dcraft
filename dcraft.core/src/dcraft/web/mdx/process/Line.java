 ...
 */
package dcraft.web.mdx.process;
...
public class Line
{
    ...
    public String readUntil(char... end)
    {
        ...
        char ch = pos < this.value.length() ? this.value.charAt(pos) : '\n';
        for(int n = 0; n < end.length; n++)
        {
            if(ch == end[n])
            {
    ...
    public LineType getLineType()
    {
        ...
        if(this.countCharsStart('!') == 2)
            return LineType.IMAGE_BLOCK;
    ...
    protected boolean checkHTML() {
        ...
        if (this.value.length() - 1 <= this.leading)
        	return false;
