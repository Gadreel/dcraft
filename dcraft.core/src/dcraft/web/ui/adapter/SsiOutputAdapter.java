	...
	public void execute(WebContext ctx) throws Exception {
		...
		while (checkmatches) {
			...
			while (m.find()) {
				...
				CommonPath pp = new CommonPath(vfilename);  //"/header-ssi.html");		
				
				IOutputAdapter sf = ctx.getSite().getWebsite().findFile(pp, ctx.isPreview());
				
				if (sf instanceof SsiOutputAdapter) {
					CharSequence val = ((SsiOutputAdapter)sf).getFile().asChars();
