...
import dcraft.ctp.stream.FileDestStream;
...
import dcraft.filestore.IFileStoreStreamDriver;
...
public class FileSystemFile extends RecordStruct implements IFileStoreFile {
	...
	public RecordStruct getExtra() {
		return new RecordStruct().withField("FullPath", this.getFullPath());
	}
	...
	public class DestinationDriver implements IFileStoreStreamDriver {						
	        ...
	        try {
	        	...
	        	if (append && exists) {
	        		...
	        		if (this.expectedsize < size) {
	        			this.fchannel.close();
	        			or.error("File size exceeds the Expected Size: " + this.expectedsize);
	        			return;
	        		}
		...
		public void message(StreamMessage msg) {
	    	...
	    	if (msg.hasData()) {
	    		...
	    		if (bb.nioBufferCount() > 0) {
					...
					try {
		        		...
		        		FileSystemFile.this.setField("Size", this.writtensize);
	    	...
	    	if (msg.isFinal()) {
				...
				FileSystemFile.this.setField("Modified", new DateTime(DateTimeZone.UTC));
				FileSystemFile.this.setField("Exists", this.writtensize > 0);
				
	    		// let them know we ended as expected
				this.channel.send(MessageUtil.streamFinal());
				
				RecordStruct res = new RecordStruct();
				res.copyFields(FileSystemFile.this);
				this.channel.setResult(res);
