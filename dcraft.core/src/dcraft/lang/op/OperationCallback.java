...
abstract public class OperationCallback extends OperationResult {
	...
	public OperationCallback(TimeoutPlan plan) {
		...
		Task timeouttask = Task.taskWithSubContext()
			.withTitle("OperationCallback with a timeout")
			.withWork(new IWork() {
			@Override
	...
	public OperationCallback(OperationContext ctx, TimeoutPlan plan) {
		...
		Task timeouttask = Task.taskWithContext(ctx)
			.withTitle("OperationCallback with a timeout and a specific context")
			.withWork(new IWork() {
			@Override
			public void run(TaskRun task) {
				OperationCallback.this.abandon();
