...
public class DownloadFile implements IWork {
		...
		if (this.xferctx == null) {
			...
			run.getContext().addObserver(new OperationObserver() {
				...
				public void completed(OperationContext ctx) {
					// TODO review
					if (StringUtil.isNotEmpty(DownloadFile.this.xferctx.channelid))
						DownloadFile.this.session.abortStream(DownloadFile.this.xferctx.channelid);
				}
			});
		}
