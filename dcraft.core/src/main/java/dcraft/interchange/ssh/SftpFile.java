...
#
************************************************************************ */
package dcraft.interchange.ssh;

public class SftpFile { /* extends RecordStruct implements IFileStoreFile {
	...
	public SftpFile() {
		this.setType(Hub.instance.getSchema().getType("dciSftpFile"));
	}

	public SftpFile(SftpDriver driver, LsEntry file, String fullpath) {
		this();
	...
	* /
    ...
    protected void doCopy(Struct n) {
    	...
    	SftpFile nn = (SftpFile)n;
		nn.driver = this.driver;
    }
    
	@Override
	public Struct deepCopy() {
		SftpFile cp = new SftpFile();
		this.doCopy(cp);
		return cp;
	}
	...
	public FuncResult<Struct> getOrAllocateField(String name) {
		if ("TextReader".equals(name) && !this.removed) {
			...
			res.setResult(new SftpTextReader(this));
			return res;
		}
	...
	* /
	...
	public class DestinationDriver implements IDestinationDriver {						
		...
		public void present(Block data) {
			try {
				...
				/*  TODO this should be in here in some sense   #NETTY
	...
	public void getInputStream(final FuncCallback<InputStream> callback) {
		...
		SftpFile.this.driver.getChannel(new FuncCallback<ChannelSftp>() {						
			@Override
			public void callback() {
				ChannelSftp channel = this.getResult();
				
				try {
					callback.setResult(channel.get(SftpFile.this.getFieldAsString("FullPath")));
				}
				catch (Exception x) {
					// TODO log
				}
				finally {
					SftpFile.this.driver.releaseChannel(channel);
				}
	...
	public void rename(final String name, final OperationCallback callback) {
		...
		SftpFile.this.driver.getChannel(new FuncCallback<ChannelSftp>() {						
			...
			public void callback() {
				...
				try {
					String newpath = SftpFile.this.getFieldAsString("FullPath");
					...
					channel.rename(SftpFile.this.getFieldAsString("FullPath"), newpath);
					
					SftpFile.this.setField("Name", name);
					SftpFile.this.setField("FullPath", newpath);
					
					newpath = SftpFile.this.getFieldAsString("Path");
					...
					SftpFile.this.setField("Path", newpath);
				}
				catch (Exception x) {
					// TODO log
				}
	...
	public void remove(final OperationCallback callback) {
		...
		SftpFile.this.driver.getChannel(new FuncCallback<ChannelSftp>() {						
			...
			public void callback() {
				...
				try {
					channel.rm(SftpFile.this.getFieldAsString("FullPath"));
					SftpFile.this.removed = true;
				}
				catch (Exception x) {
					// TODO log
				}
		...
		SftpFile.this.driver.getChannel(new FuncCallback<ChannelSftp>() {						
			...
			public void callback() {
				...
				try {
					...
					SftpFile.this.file.getAttrs().setACMODTIME(t, t);
					channel.setStat(SftpFile.this.getFieldAsString("FullPath"), SftpFile.this.file.getAttrs());
					SftpFile.this.setField("Modified", time);
				}
				catch (Exception x) {
					// TODO log
				}
