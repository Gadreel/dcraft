...
public class Bucket {
	...
	public void init(SiteInfo di, XElement bel, OperationCallback cb) {
	...
	public boolean checkUploadToken(RecordStruct data, IFileStoreFile file) {
		if (!this.uploadtoken)
			return true;
	...
	public void handleFileDetail(RecordStruct request, FuncCallback<RecordStruct> fcb) {
		...
		this.mapRequest(request, new FuncCallback<IFileStoreFile>() {
			...
			public void callback() {
				...
				fdata.setField("Extra", fi.getExtra());
	...
	public void handleAllocateUploadToken(RecordStruct request, FuncCallback<RecordStruct> fcb) {
		RecordStruct resp = new RecordStruct();
		fcb.setResult(resp);
		...
		scache.put(token, new BooleanStruct(true));
		
		resp.setField("Token", token);
	...
	public void handleStartUpload(RecordStruct request, FuncCallback<RecordStruct> fcb) {
		// check bucket security
		if (!this.checkWriteAccess() && !this.uploadtoken) {
			fcb.errorTr(434);
			fcb.complete();
			return;
		}
		...
		this.mapRequest(request, new FuncCallback<IFileStoreFile>() {
			...
			public void callback() {
				...
				if (!Bucket.this.checkUploadToken(request, fi)) {
					fcb.error("You may not upload to this path.");
					fcb.complete();
					return;
				}
