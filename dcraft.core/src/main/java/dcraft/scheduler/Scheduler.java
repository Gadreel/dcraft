...
public class Scheduler {
	...
	public void init(OperationResult or, XElement config) {
		...
		if (config != null) {
			...
			if (StringUtil.isEmpty(classname))
				classname = "dcraft.scheduler.LocalSchedulerDriver";
		...
		if (this.driver != null) {
			...
			if (loadres.isNotEmptyResult()) {				
				loadres.getResult().recordStream().forEach(rec -> {
					...
					sched.setTask(Task.taskWithRootContext()
    ...
    public ISchedule runNow(Task task) {
    	return this.addNode(new SimpleSchedule(task, DateTimeUtils.currentTimeMillis(), 0));
    }

	// run the work unit once in Sec seconds from now
	public ISchedule runIn(Task task, int secs) {
		return this.addNode(new SimpleSchedule(task, DateTimeUtils.currentTimeMillis() + (1000 * secs), 0));		
	}
	...
	public ISchedule runAt(Task task, ReadableInstant time) {
		return this.addNode(new SimpleSchedule(task, time.getMillis(), 0));				
	}
	
	public ISchedule runAt(Task task, LocalDate date, Period period) {
		LocalDateTime ldt = date.toLocalDateTime(new LocalTime(0, 0).plus(period));
		return this.runAt(task, ldt.toDateTime());
	}

	// run the work unit repeatedly, every Secs seconds - note scheduler will not own work, you have to keep track of it
	public ISchedule runEvery(Task task, int secs) {
		return this.addNode(new SimpleSchedule(task, DateTimeUtils.currentTimeMillis() + (1000 * secs), secs));		
	}
