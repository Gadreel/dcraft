...
public class OperationContext implements ITranslationAdapter {
	...
	*/
	...
	static public OperationContext allocateSiteRoot() {
		// for occasions where no context is set when calling allocate - we need some context
		if (!OperationContext.hasContext())
			OperationContext.context.set(OperationContext.defaultcontext);
		...
		return new OperationContextBuilder()
			...
			.withTenantId(ctx.getUserContext().getTenantId())
			.withSite(ctx.getUserContext().getSiteAlias())
			.toOperationContext();
	}
    ...
    public void touch() {
    	...
    	// TODO use this or use deep Touch + logger/progress events?
    	if (this.parent != null)
    		this.parent.touch();
    }
	...
	public boolean hasCode(long code, int msgStart, int msgEnd) {
		// TODO review that the logOffset concept works correctly and that the bounds checker is fine for all methods in this class
		msgStart -= this.logOffset;		// adjust so the markers are relative to the current collection of messages, assuming some may have been purged
	...
	public String getLog() {
		try {
			...
			System.out.println("Error with logger: " + x);
	...
	public void addObserver(IOperationObserver oo) {
		...
		// TODO this doesn't work, too concerned about duplicates
		...
		try {
			for (IOperationObserver ob : this.observers) {
		    	...
		    	try {
		    		...
		    		System.out.println("Unhealthy Observer " + ob.getClass().getName() + " - " + x);
		    	}
			}
