...
public class DatabaseManager implements IDatabaseManager {
	...
	public void submit(IDatabaseRequest request, DatabaseResult cb) {
		...
		req.setField("Tenant", request.hasTenant() ? request.getTenant() : cb.getContext().getUserContext().getTenantId());
	...
	public void backup() throws RocksDBException {
		...
		for (BackupInfo info : this.backupEngine.getBackupInfo()) 
			System.out.println("- backup: " + info.backupId() + " -  " + info.timestamp());
		
        this.backupEngine.createNewBackup(db);
		
		this.backupEngine.purgeOldBackups(14);		// keep up to 2 weeks of backups
