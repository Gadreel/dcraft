	...
	public OperationResult setFields(String table, String id, RecordStruct fields) {
		...
		for (FieldStruct field : fields.getFields()) {
			...
			try {
				...
				DataType dtype = TablesAdapter.this.task.getSchema().getType(schema.getTypeId()); 
				...
				if (!schema.isList() && !schema.isDynamic()) {
					...
					if (retired) {
						if (oldIsSet && auditDisabled) {
							...
							this.conn.kill(DB_GLOBAL_RECORD, did, table, id, fname, stamp, "Search");
						}
						...
						if ((ctype != null) && (ctype.getType() == RootType.String)) {
							...
							this.conn.set(DB_GLOBAL_RECORD, did, table, id, fname, stamp, "Search", sValue);
							 */
						}
					}
 ...
 . i (filter'="")&(^dcProg("wherefilter",filter)'="") x "s res=$$"_^dcProg("wherefilter",filter)_"()"
		...
		for (int i = 0; i < parts.length; i++) {
			...
			if (StringUtil.isNotEmpty(pfname)) {
				DbField schema = this.task.getSchema().getDbField(table, pfname);
				
				if (schema == null) {
					OperationContext.get().error("Field not defined: " + table + " - " + pfname);
					continue;
				}
				...
				if (dtype == null) {
					OperationContext.get().error("Field type missing: " + table + " - " + pfname);
					continue;
				}
				...
				// TODO until we support the "Search" feature of Text values we need to treat strings differently
				else if ((ctype != null) && (ctype.getType() == RootType.String)) {
					ArrayList<byte[]> vl = new ArrayList<>();
					List<Object> vals = this.get(table, id, pfname, subid, when, null, historical);
					
					// string to lower, then to byte
					for (Object val : vals) {
						if (val != null)
							vl.add(ByteUtil.buildValue(val.toString().toLowerCase()));
					}
			...
			Object vc = pdef.getFieldAsAny("_ValueCache");
			
			if (vc instanceof ArrayList<?>) {
				values.set(i, (List<byte[]>) vc);
				continue;
			}
			...
			if (val != null) {
				...
				if (val instanceof ListStruct) {
					...
					for (int i2 = 0; i2 < lval.getSize(); i2++) {
						Struct vi = lval.getItem(i2);
						
						// TODO until we support the "Search" feature of Text values we need to treat strings differently
						if (vi instanceof StringStruct) 
							vl.add(ByteUtil.buildValue(vi.toString().toLowerCase()));
						else 
							vl.add(ByteUtil.buildValue(vi));
					}
				}
				// TODO until we support the "Search" feature of Text values we need to treat strings differently
				else if (val instanceof StringStruct) {
					vl.add(ByteUtil.buildValue(val.toString().toLowerCase()));
				}
				else {
		...
		if ("StartsWith".equals(expression)) {
			...
			for (int i = 0; i < a.size(); i++) {
				for (int i2 = 0; i2 < b.size(); i2++) {
					if (ByteUtil.dataStartsWith(a.get(i), b.get(i2)))
						return true;
				}
			}
		...
		if ("EndsWith".equals(expression)) {
			...
			for (int i = 0; i < a.size(); i++) {
				for (int i2 = 0; i2 < b.size(); i2++) {
					if (ByteUtil.dataEndsWith(a.get(i), b.get(i2)))
						return true;
				}
			}
		...
		if ("Contains".equals(expression)) {
			...
			for (int i = 0; i < a.size(); i++) {
				for (int i2 = 0; i2 < b.size(); i2++) {
					if (ByteUtil.dataContains(a.get(i), b.get(i2)))
						return true;
				}
			}
	...
	public void traverseIndex(String table, String fname, Object val, BigDateTime when, boolean historical, Function<Object,Boolean> out) {
