...
public class EncryptedFileStream {
    ...
    public void addPublicKey(PGPPublicKeyRing ring) {
    	if (ring == null)
        ...
        if (!pubKey.isEncryptionKey())
        	throw new IllegalArgumentException("Public key is not an encryption key.");
    	
		this.methods.add(new BcPublicKeyKeyEncryptionMethodGenerator(pubKey));
	}
    ...
    public void loadPublicKey(Path keyring) throws IOException, PGPException {
    	...
    	PGPPublicKeyRingCollection pgpPub = new PGPPublicKeyRingCollection(org.bouncycastle.openpgp.PGPUtil.getDecoderStream(keyIn), new BcKeyFingerprintCalculator());
	...
	public void writeFirstLiteral(int dataLength) {
        ...
        // TODO this number does not come out right, fix
