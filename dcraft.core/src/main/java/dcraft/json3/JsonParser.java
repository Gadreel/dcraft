...
package dcraft.json3;

import java.io.*;
import java.math.BigDecimal;
import java.math.BigInteger;

import dcraft.io.CharSequenceReader;
import dcraft.json3.base.ReaderBasedJsonParser;
import dcraft.json3.io.IOContext;
import dcraft.json3.util.BufferRecycler;
...
{
	static public ReaderBasedJsonParser createParser(InputStream in) throws IOException {
		IOContext ctx = new IOContext(new BufferRecycler(), in, false);
		
		/* TODO maybe detect encoding someday
        JsonEncoding enc = ctx.getEncoding();
        
        Reader rdr = new InputStreamReader(in, enc.getJavaName());
		*/
		
        Reader rdr = new InputStreamReader(in, "UTF-8");
		
        return new ReaderBasedJsonParser(ctx, rdr);
	}
	
	static public ReaderBasedJsonParser createParser(CharSequence in) throws IOException {
		IOContext ctx = new IOContext(new BufferRecycler(), in, false);
        
        CharSequenceReader rdr = new CharSequenceReader(in);
