...
public class Processor {
    ...
    public static UIElement process(IOutputContext ctx, String input, Configuration configuration) throws IOException {
    	try (StringReader in = new StringReader(input)) {
    		return process(ctx, in, configuration);
    	}
    }
    
    public static UIElement parse(ProcessContext ctx, String input) throws IOException {
    	try (StringReader in = new StringReader(input)) {
    		return parse(ctx, in);
    	}
    }
    
    public static UIElement parse(ProcessContext ctx, Reader reader) throws IOException {
    	try {
    ...
    public Processor(IOutputContext ctx, Reader reader, Configuration config) {
    	this.ctx = new ProcessContext(config, ctx);
        this.reader = reader;
        this.emitter = new Emitter(this.ctx);
    }

    public UIElement toXml() throws IOException {
    	UIElement root = new UIElement("div");		
